...
class AxialComponent extends React.PureComponent {
  constructor (...args) {
    super(...args);
  ...
  shouldComponentUpdate (nextProps, nextState) {
    ...
    for (let path in state) {
      if (state.hasOwnProperty(path)) {
        const binding = this._bindings.find(b => b.toString() === path);
        if (binding) {
...
AxialComponent.onDebugRender = (output, component) => {
  ...
  return <div style={{backgroundColor:'rgb(' + [r,g,b].join(',') + ')',border:'3px dashed rgba(0,0,0,0.1)',margin:'5px',padding:'5px'}}><b style={style}>{component.constructor.name}</b>{output}</div>;
};
