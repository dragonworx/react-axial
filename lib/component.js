...
class AxialComponent extends React.Component {
  ...
  _render () {
    ...
    // render output, but wrap in debugging components (rendering will also create bindings as instance properties are read)
  ...
  enterRender () {
    // clear previous bindings from instances to this, would have been created from previous render
    ...
    Axial.bind(this.captureBinding.bind(this));
  ...
  exitRender () {
    ...
    Axial.unbind(this.captureBinding.bind(this));
  }
  ...
  onInstanceChange (eventData) {
    ...
    if (eventData.method === 'set') {
      console.log(`%cCHANGE:[${eventData.property.path}] old: ${JSON.stringify(eventData.oldValue)} new: ${JSON.stringify(eventData.newValue)}`, 'color:blue');
