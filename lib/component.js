...
const TRANS_ID_KEY = 'data-trans-id';
const TRANSITION_END_EVENT = (function () {
  let t;
...
class AxialComponent extends React.Component {
  ...
  enterRender () {
    ...
    for (let id in this._transitionListeners) {
      if (this._transitionListeners.hasOwnProperty(id)) {
        const handler = this._transitionListeners[id];
        document.body.removeEventListener(TRANSITION_END_EVENT, handler, true);
      }
    }
  ...
  /**
   * register a callback with a transition id
   * @param id
   * @param fn
   */
  onTransitionEnd (id, propertyName, value, fn) {
    const handler = function (e) {
      const transKey = AxialComponent.getParentWithAttr(e.target, TRANS_ID_KEY);
      const transId = transKey ? transKey.getAttribute(TRANS_ID_KEY) : 'null';
      const property = e.propertyName;
      const currentValue = parseFloat(getComputedStyle(e.target)[property]);
      document.body.removeEventListener(TRANSITION_END_EVENT, handler, true);
      if ('' + id === transId && propertyName === property && currentValue === value) {
        const dataInstanceElement = AxialComponent.getParentWithAttr(e.target, 'data-id');
        if (dataInstanceElement) {
          let instance = Axial.getInstance(dataInstanceElement.getAttribute('data-id'));
          fn(instance, {
            originalEvent: e,
            target: e.target,
            id: transId
    ...
    document.body.addEventListener(TRANSITION_END_EVENT, handler, true);
    this._transitionListeners[id] = handler;
  }
