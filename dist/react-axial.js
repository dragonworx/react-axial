...
var Axial = __webpack_require__(33);
var _currentRenderingComponent = [];
var _transitions = {};
var _keyId = 0;
...
var AxialComponent = function (_React$PureComponent) {
  _inherits(AxialComponent, _React$PureComponent);
  ...
  function AxialComponent() {
    ...
    var _this = _possibleConstructorReturn(this, (_ref = AxialComponent.__proto__ || Object.getPrototypeOf(AxialComponent)).call.apply(_ref, [this].concat(args)));

    _this._bindings = [];
    _this._transitionListeners = {};

    _this.__render = _this.render;
  ...
  _createClass(AxialComponent, [{
    ...
    value: function shouldComponentUpdate(nextProps, nextState) {
      ...
      // ensure nextState has latest values of matching bindings
      var state = this.state;

      var _loop = function _loop(path) {
        if (state.hasOwnProperty(path)) {
          var binding = _this2._bindings.find(function (b) {
            return b.toString() === path;
          });
          if (binding) {
            if (!nextState.hasOwnProperty(path)) {
              throw new Error('!!!');
            }
            nextState[path] = binding.get();
          }
        }
      };
      ...
      // figure out whether any bindings are different
      var isEqual = true;
      for (var path in state) {
        if (state.hasOwnProperty(path)) {
          var currentValue = state[path];
          var nextValue = nextState[path];
          var notEqual = currentValue !== nextValue;
    ...
    key: 'captureBindingDuringRender',
    value: function captureBindingDuringRender(eventData) {
      // only use GET
      if (eventData.method === 'get' && typeof eventData.value !== 'function') {
        ...
        if (this._bindings.find(function (binding) {
          return binding.instance === instance && binding.key === key;
        })) {
          return;
        }
        // get this component to re-render every time the instance property changes (to a new unique value)
        var binding = new Axial.Binding(instance, key, this.onInstanceChange.bind(this));
    ...
    value: function onInstanceChange(eventData) {
      // only use SET
    ...
    value: function enterRender() {
      ...
      for (var id in this._transitionListeners) {
        if (this._transitionListeners.hasOwnProperty(id)) {
          var info = this._transitionListeners[id];
    ...
    value: function onTransitionEnd(fn) {
      var TRANSITION_EVENT = _transitions.end;
      var transitionKey = '' + _transitionId++;
      var handler = function (e) {
        var property = e.propertyName;
        var value = parseFloat(getComputedStyle(e.target)[property]);
        var targetId = e.target.getAttribute('data-transition-end');
    ...
    key: 'getParentWithAttr',
    value: function getParentWithAttr(target, attrName) {
      var ref = target;
...
AxialComponent.onDebugRender = function (output, component) {
  ...
  return React.createElement(
    'div',
    { style: { backgroundColor: 'rgb(' + [r, g, b].join(',') + ')', border: '3px dashed rgba(0,0,0,0.1)', margin: '5px', padding: '5px' } },
    React.createElement(
      'b',
      { style: style },
...
AxialComponent.addDefaultLogListeners = function () {
  Axial.addLogListener('component.bindings.changed', function (e) {
    console.log('%c[' + e.component.constructor.name + '.' + e.path + '] bindings changed', 'color:red');
  }).addLogListener('component.bindings.changed.equal', function (e) {
    console.log('%c[' + e.component.constructor.name + '] bindings are equal, should not update', 'color:green');
  }).addLogListener('component.bindings.changed.difference', function (e) {
    console.log('%c[' + e.component.constructor.name + '] bindings are not equal, should update', 'color:red');
  }).addLogListener('component.render.start', function (e) {
    console.group('RENDER:[' + e.component.constructor.name + ']');
  }).addLogListener('component.render.end', function (e) {
    console.groupEnd();
  }).addLogListener('component.binding.changed', function (e) {
    console.log('%cCHANGE:[' + e.property.path + '] old: ' + Axial.util.stringify(e.oldValue) + ' new: ' + Axial.util.stringify(e.newValue), 'color:blue');
  }).addLogListener('component.binding.equal', function (e) {
    console.log('%cEQUAL:[' + e.property.path + ']', 'color:green');
  }).addLogListener('component.binding.difference', function (e) {
    console.log('%cDIFFERENCE:[' + e.property.path + ']', 'color:red');
  }).addLogListener('component.render.bindings', function (e) {
    console.log('%cBindings: [' + e.component.constructor.name + ']', 'font-style:italic;color:#ccc');
  }).addLogListener('component.render.binding', function (e) {
    console.log('%c' + e.binding.index + '. ' + e.binding.instance.toString() + '~["' + e.binding.property.key + '"]:' + e.binding.property.types.toString(), 'color:blue;padding-left:20px;');
  });
};

Axial.Component = AxialComponent;

Axial.Component.debug = function (onDebugRender) {
  Axial.debug();
  Axial.Component.addDefaultLogListeners();
  if (onDebugRender) {
    Axial.Component.debug = true;
    AxialComponent.onDebugRender = onDebugRender;
  }
};
...
/***/ })
/******/ ]);
//# sourceMappingURL=react-axial.js.map